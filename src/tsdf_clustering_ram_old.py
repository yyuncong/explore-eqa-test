...
from .detection_utils import detection_metric, gt_class_match
...
class TSDFPlanner(TSDFPlannerBase):
    ...
    def update_scene_graph(
        ...
        print(all_classes)
        transform = get_transform(image_size=cfg.recognize_img_size)
        # the number of recognized classes are too little
        # possible solutions:
        # 1. revise the confidence threshold of RAM model
        # 2. use the union of all_classes and recognized_class
        with torch.no_grad():
            tags,_ = recognize_model.generate_tag(transform(Image.fromarray(rgb)).unsqueeze(0))
        recognized_tags = list(tags[0].split(' | '))
        # filter out tags that are not corresponding to concrete objects
        recognized_class = []
        for tag in recognized_tags:
            is_object = True
            for key_word in ['wall', 'floor', 'ceiling', 'room', 'space']:
                if key_word in tag:
                    is_object = False
                    break
            if is_object:
                recognized_class.append(tag)
        ...
        # so the detection model matches the object with given classes?
        if cfg.use_ram:
            all_classes = recognized_class
        detection_model.set_classes(all_classes)
        #detection_model.set_classes(recognized_class)
        ...
        for i in range(len(detections)):
            info = {}
            info['class_name'] = all_classes[detections.class_id[i]]
        ...
        if cfg.use_gt_class:
            for i in range(len(detection_infos)):
                dinfo = detection_infos[i]
                obj_ids = class_to_obj_id[dinfo['class_name']]
                match_obj,caption,bbox_center = gt_class_match(dinfo,
                    obj_ids, obj_id_to_bbox, 
                    semantic_obs, obs_point, cfg.obj_include_dist,
                    cfg.iou_threshold)
                if match_obj is not None:
                    frame.full_obj_list[match_obj] = dinfo['confidence']
                    if match_obj not in self.simple_scene_graph.keys():
                        self.simple_scene_graph[match_obj] = SceneGraphItem(
                            object_id=match_obj,
                            bbox_center=bbox_center,
                            confidence=dinfo['confidence'],
                            image=None,
                            classes={dinfo['class_name']:1},
                            gt_class = obj_id_to_name[match_obj]
                            )
                        if match_obj not in self.scene_graph_list:
                            self.scene_graph_list.append(match_obj)
                        caption += "_N"
                    ...
                    detected_class.append(
                        {
                            "recognize_name": dinfo['class_name'],
                            "gt_name": obj_id_to_name[match_obj]
                        }
                    )
        else:
            for obj_id in all_obj_id:
                ...
                bbox = np.asarray(obj_id_to_bbox[obj_id]["bbox"])
        ...
        '''
        for i in range(len(detections)):
            # the class_name of the ith detection result
            ...
            for obj_id in all_obj_id:     
                # from detection result, we know this region belongs to a specific object
                # but we not sure which object it is
                # the we go through all the objects belonging to this class
                # and try to match the bbox
                if obj_id not in obj_id_to_bbox.keys():
                    continue
                ...
                obj_mask = np.zeros(semantic_obs.shape, dtype=bool)
                obj_mask[obj_x_start:obj_x_end, obj_y_start:obj_y_end] = True
                # if the iou score is larger than a specific threshold
                # we accept this object
