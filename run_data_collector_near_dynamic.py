...
from src.tsdf_clustering_near_dynamic import TSDFPlanner, Frontier, SnapShot
        ...
        for question_data in all_questions_in_scene:
            ...
            for path_idx in range(cfg.path_id_offset, cfg.path_id_offset + cfg.paths_per_question):
                ...
                while explore_dist < max_explore_dist and cnt_step < max_step:
                    ...
                    tsdf_planner.update_snapshots(obj_ids=set(all_added_obj_ids), min_detections=cfg.scene_graph.obj_min_detections)
                    ...
                    min_detection = cfg.scene_graph.obj_min_detections
                    obj_exclude_count = sum([1 if sum(obj.classes.values()) < min_detection else 0 for obj in tsdf_planner.simple_scene_graph.values()])
                    ...
                    for obj_id in tsdf_planner.simple_scene_graph.keys():
                        ...
                        for ss in tsdf_planner.snapshots.values():
                            if obj_id in ss.cluster:
                                exist_count += 1
                        if sum(tsdf_planner.simple_scene_graph[obj_id].classes.values()) < min_detection:
