...
class TSDFPlanner(TSDFPlannerBase):
    ...
    def update_scene_graph(
        ...
        # extract all classes
        all_classes_gt = list(class_to_obj_id.keys())
        all_obj_id = set(itertools.chain(*[v for v in class_to_obj_id.values()]))
        ...
        # initialize a SnapShot, get rgb and point observation
        frame = SnapShot(
            image=file_name,
        ...
        caption_list = []
        detection_infos = []
        # record basic information of the detection results
        for i in range(len(detections)):
            info = {}
            info['class_name'] = class_id_to_name[detections.class_id[i]]
            info['confidence'] = float(detections.confidence[i])
        ...
        # go through all ground truth objects to see if they are correctly detected
        detected_class = []
        new_obj_ids = []
        for obj_id in all_obj_id:
            n_pix = np.sum(semantic_obs == obj_id)
            if n_pix / (semantic_obs.shape[0] * semantic_obs.shape[1]) < cfg.obj_include_pix_ratio:
                continue

            # get the bbox center from semantic segmentation
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            cam_K_tensor = torch.from_numpy(intrinsics[:3, :3]).to(device).float()
            depth_tensor = torch.from_numpy(depth).to(device).float()  # (H, W)
            mask_tensor = torch.from_numpy((semantic_obs == obj_id).astype(int)).to(device).float()
            trans_pose_tensor = torch.tensor(cam_pos).to(device).float()

            # pixel to camera coordinate
            H, W = mask_tensor.shape
            fx, fy, cx, cy = cam_K_tensor[0, 0], cam_K_tensor[1, 1], cam_K_tensor[0, 2], cam_K_tensor[1, 2]
            _x, _z = torch.meshgrid(torch.arange(0, W, device=device), torch.arange(H - 1, -1, -1, device=device), indexing='xy')
            y = depth_tensor * mask_tensor
            valid = (y > 0).float()  # Mask out zeros
            x = (_x - cx) * y / fx
            z = (_z - cy) * y / fy
            points = torch.stack((x, z, -y), dim=-1) * valid.unsqueeze(-1)  # (H, W, 3)

            valid_point_mask = points[..., 2] < 0
            valid_points = points[valid_point_mask]  # (N, 3)

            # downsample the points
            valid_points = valid_points[::10]

            # camera to world coordinate
            valid_points = torch.cat((valid_points, torch.ones_like(valid_points[: , :1])), dim=1).transpose(0, 1)  # (4, N)
            valid_points = trans_pose_tensor @ valid_points
            valid_points = valid_points[:3, :].transpose(0, 1)  # (N, 3)

            n_points = valid_points.shape[0]
            bbox_center = valid_points.mean(dim=0).cpu().numpy()

            if np.linalg.norm(bbox_center[[0, 2]] - obs_point[[0, 2]]) > cfg.obj_include_dist:
                continue
            ...
            max_score,macthed_idx = 0,-1
            for i, dinfo in enumerate(detection_infos):
                # the detection is already matched
                if i in adopted_indices:
                    continue
                iou = IoU(dinfo['bbox_mask'], obj_mask)
                score = detection_metric(iou,dinfo['confidence'],measure=cfg.detection_measure)
                if score > max_score:
                    max_score = score
                    matched_idx = i
            if max_score > detection_metric(cfg.iou_threshold,cfg.confidence_threshold,measure=cfg.detection_measure):
                dinfo = detection_infos[matched_idx]
                caption = f"{obj_id}_{dinfo['class_name']}_{dinfo['confidence']:.3f}"
                frame.full_obj_list[obj_id] = dinfo['confidence']
                if obj_id not in self.simple_scene_graph.keys():
                    self.simple_scene_graph[obj_id] = SceneGraphItem(
                        object_id=obj_id,
                        bbox_center=bbox_center,
                        confidence=dinfo['confidence'],
                        image=None,
                        classes={dinfo['class_name']:1},
                        gt_class = obj_id_to_name[obj_id],
                        n_points=n_points,
                        n_detections=1
                    )
                    new_obj_ids.append(obj_id)
                else:
                    # update the recognized class
                    recognize_class = dinfo['class_name']
                    if recognize_class not in self.simple_scene_graph[obj_id].classes.keys():
                        self.simple_scene_graph[obj_id].classes[recognize_class] = 1
                    else:
                        self.simple_scene_graph[obj_id].classes[recognize_class] += 1
                    self.simple_scene_graph[obj_id].n_detections += 1

                    # update the confidence
                    self.simple_scene_graph[obj_id].confidence = max(self.simple_scene_graph[obj_id].confidence, dinfo['confidence'])

                    # update the bbox center
                    old_n_points = self.simple_scene_graph[obj_id].n_points
                    self.simple_scene_graph[obj_id].bbox_center = (self.simple_scene_graph[obj_id].bbox_center * old_n_points + bbox_center * n_points) / (old_n_points + n_points)
                    self.simple_scene_graph[obj_id].n_points += n_points

                current_obj_name, count = '',0
                for class_name_,count_ in self.simple_scene_graph[obj_id].classes.items():
                    if count_ > count:
                        current_obj_name = class_name_
                        count = count_
                detected_class.append(
                    {
                        "recognize_name": current_obj_name,
                        "gt_name": obj_id_to_name[obj_id]
                    }
                )
                adopted_indices.append(matched_idx)

                caption += f"; {self.simple_scene_graph[obj_id].n_detections}_{current_obj_name}_{self.simple_scene_graph[obj_id].confidence:.3f}"
    ...
    def periodic_cleanup_objects(self, pts, min_distance, min_detections):
        obj_to_pop = []
        for obj_id, obj in self.simple_scene_graph.items():
            if np.linalg.norm(obj.bbox_center[[0, 2]] - pts[[0, 2]]) > min_distance:
                if obj.n_detections < min_detections:
                    obj_to_pop.append(obj_id)
        for obj_id in obj_to_pop:
            logging.info(f"clean up {obj_id} in periodic_clup_objects")
            self.simple_scene_graph.pop(obj_id)
        ...
        for filename, ss in self.frames.items():  # TODO: check whether content in snapshots are also changed, and see whether need to remove snapshot that have empty cluster
            ss.cluster = [obj_id for obj_id in ss.cluster if obj_id in self.simple_scene_graph.keys()]
            ss.full_obj_list = {obj_id: conf for obj_id, conf in ss.full_obj_list.items() if obj_id in self.simple_scene_graph.keys()}
            if len(ss.full_obj_list) == 0:
                frame_to_pop.append(filename)
        for filename in frame_to_pop:
            self.frames.pop(filename)
            if filename in self.snapshots.keys():
    ...
    def update_snapshots(
        ...
        obj_exclude = [obj_id for obj_id in self.simple_scene_graph.keys() if sum(list(self.simple_scene_graph[obj_id].classes.values())) < min_detections]
        ...
        # sanity check
        for obj_id, obj in self.simple_scene_graph.items():
            if sum(list(self.simple_scene_graph[obj_id].classes.values())) < min_detections:
    ...
    def get_next_choice(
        ...
        if target_obj_id in self.simple_scene_graph.keys() and sum(self.simple_scene_graph[target_obj_id].classes.values()) >= min_detections:
