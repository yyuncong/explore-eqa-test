    ...
    for question_idx, question_data in enumerate(questions_list):
        ...
        while cnt_step < num_step - 1:
            ...
            if type(max_point_choice) == SnapShot:
                ...
                if target_arrived:
                    main_view_index = total_views // 2
                    main_obs_file_name = f"{cnt_step}-view_{main_view_index}.png"
                    for i in range(max(0, cnt_step - 5), cnt_step + 1):
                        os.system(f"cp {os.path.join(episode_snapshot_dir, main_obs_file_name)} {os.path.join(episode_object_observe_dir, f'obs_{i}.png')}")
