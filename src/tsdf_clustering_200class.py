...
class TSDFPlanner(TSDFPlannerBase):
    ...
    def update_scene_graph(
        ...
        for obj_id in unique_obj_ids:
            ...
            if obj_id == 0 or obj_id not in gt_obj_id_to_name.keys() or gt_obj_id_to_name[obj_id] in ['wall', 'floor', 'ceiling', 'door frame']:
                continue
            # extend the class_to_obj_id dict
            if gt_obj_id_to_name[obj_id] not in class_to_obj_id.keys():
                class_to_obj_id[gt_obj_id_to_name[obj_id]] = [obj_id]
            else:
                class_to_obj_id[gt_obj_id_to_name[obj_id]].append(obj_id)
        # extract all classes
        ...
        for i in range(len(detections)):
            info = {}
            info['class_name'] = scannet_class_id_to_name[detections.class_id[i]]
                ...
                detected_class.append(
                    {
                        ...
                        "gt_name": gt_obj_id_to_name[obj_id]
                    }
                )
