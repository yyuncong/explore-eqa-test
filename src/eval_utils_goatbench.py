...
def prepare_prompt_before_snapshot(step,question_text,question_feature):
    num_visual_tokens = step["num_visual_tokens"]
    #question_text, question_feature = prepare_question_prompt(step)
    #text =  f"Question: {step['question']}\n" 
    text = question_text
    multi_src_feature = [question_feature]
...
def prepare_prefiltering_prompt(
    question_text,
    question_feature, 
    tokenizer, 
    classes, 
    ...
    if question_feature is not None:
        filter_feature = question_feature
        filter_insert_loc = (
            (filter_input_ids == scene_token_id).nonzero()[:, 1].reshape(-1)
        )
    else:
        filter_feature = torch.empty((0, 1024))
        filter_insert_loc = torch.empty(0)
    filter_input_dict = EasyDict(
...
def collate_prefilter_wrapper(batch):
    ...
    max_feature_length = max(b.filter_feature.shape[0] for b in batch)
    
    filter_feature = torch.zeros((len(batch), max_feature_length, 1024))
    filter_insert_loc = torch.zeros((len(batch), max_feature_length))
    for (j,b) in enumerate(batch):
    ...
    return EasyDict(
        ...
        feature_length=torch.tensor([b.filter_feature.shape[0] for b in batch]),
...
def get_item(tokenizer, step_dict):
    ...
    question_text, question_feature = prepare_question_prompt(step)
    text_before_snapshot, feature_before_snapshot = prepare_prompt_before_snapshot(
        step, question_text, question_feature
    )
    ...
    if step.get("use_prefiltering") is True:
        ...
        filter_input_dict = prepare_prefiltering_prompt(
            question_text,
            question_feature,
            tokenizer,
            seen_classes,
        ...
        '''
        filter_input_dict.selection_dict = selection_dict
        batch = [filter_input_dict]
        return collate_prefilter_wrapper([filter_input_dict])
    else:
