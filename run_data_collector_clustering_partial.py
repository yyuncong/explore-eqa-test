...
def main(cfg):
    ...
    for scene_id in all_scene_list:
        ...
        # all_questions_in_scene = [q for q in all_questions_in_scene if '00141-iigzG1rtanx_249_lamp_899290' in q['question_id']]
        ...
        for question_data in all_questions_in_scene:
            ...
            frontier_target_detected_record = {}
            ...
            final_target_obj_id = question_data['object_id']
            ...
            if start_position is None or path_points is None:
                logging.info(f"Cannot find a navigable path to the target object in question {prev_question_data['question_id']}-path {path_idx}!")
                continue
            ...
            while explore_dist < max_explore_dist and cnt_step < 40:  # 40 is just a not normal step number to avoid agent stuck in a place
                ...
                for i, frontier in enumerate(tsdf_planner.frontiers):
                    ...
                    if frontier.image is not None:
                        assert os.path.exists(os.path.join(episode_frontier_dir, frontier.image)), f"{os.path.join(episode_frontier_dir, frontier.image)}"
                    else:
                        ...
                        # here we detect again to check whether the final target object is in the view
                        _, final_target_detected = get_frontier_observation_and_detect_target(
                            agent, simulator, cfg.scene_graph, detection_model,
                            target_obj_id=final_target_obj_id, target_obj_class=object_id_to_name[final_target_obj_id],
                            view_frontier_direction=view_frontier_direction, init_pts=pts
                        )
                        frontier_target_detected_record[f"prev_{cnt_step}_{i}.png"] = final_target_detected
                ...
                if not update_success:
                    logging.info(
                        f"Question id {question_data['question_id']}-path {path_idx} invalid: find next navigation point failed!")
                    break
                ...
                if cfg.save_frontier_video_prev:
                    ...
                    for h_idx in range(side_length):
                        for w_idx in range(side_length):
                            ...
                            if (i < num_images - 1) or (i < num_images and type(max_point_choice) == Frontier):
                                ...
                                if type(max_point_choice) == Frontier and max_point_choice.image == \
                                        tsdf_planner.frontiers[i].image:
                ...
                if type(max_point_choice) == SnapShot and target_arrived:
                    ...
                    plt.imsave(os.path.join(target_obs_save_dir, f"prev_{cnt_step}_target_observation.png"), rgb)
                    break
            ...
            # refresh the target_included flag in each frontier
            for frontier in tsdf_planner.frontiers:
                if frontier_target_detected_record[frontier.image]:
                    frontier.target_detected = True
                else:
                    frontier.target_detected = False
